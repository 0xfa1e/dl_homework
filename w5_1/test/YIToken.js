  ...
  describe("YIToken", function () {
    ...
    describe("Deployment", function () {
      ...
      it("permitDeposit", async function () {
        ...
        const {v, r, s,deadline} = await sig(otherAccount,yi,vault,1000);
      ...
      it("withdrewToOwner success", async function () {
        const { yi, vault,owner,otherAccount} = await loadFixture(deployYIToken);
        var amount = ethers.BigNumber.from("1000000000000000000000");
        await yi.transfer(otherAccount.address, amount);
        
        const {v, r, s,deadline} = await sig(otherAccount,yi,vault,amount);

        await vault.connect(otherAccount).permitDeposit(yi.address, amount, deadline,v, r, s);

        await vault.withdrewToOwner(yi.address);

        expect(await vault.totalBalanceOf(yi.address)).to.be.equal(0);
      });
      
      it("withdrewToOwner fail", async function () {
        const { yi, vault,owner,otherAccount} = await loadFixture(deployYIToken);
        var amount = ethers.BigNumber.from("99000000000000000000");
        await yi.transfer(otherAccount.address, amount);
        ...
        expect(await vault.totalBalanceOf(yi.address)).to.not.equal(0);
      });
